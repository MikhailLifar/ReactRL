...
def main():
    ...
    #         fourier_policy.set_policy({'a_sin': random_coefs_0[:terms_number],
    #                                    'a_cos': random_coefs_0[terms_number:],
    #                                    'length': length})
    #
    #     return time_dependence_, callback_

    def get_random_turns_time_dependence(period, bounds, delta):
        rtp = RandomTurnsPolicy({'period': period, 'bounds': bounds})

        def time_dependence_(act, t):
            return np.array([act[0], rtp(t + delta)])
    ...
    def eval_agent(agent, env):
        ...
        rews = np.empty(len(CO_curves))
        for i, curve in enumerate(CO_curves):
    ...
    def test_callback(env, iteration):
        if iteration >= len(CO_curves):
            env.time_input_dependence = test_cache.pop()
            return False
        if not test_cache:
            test_cache.append(env.time_input_dependence)
        env.time_input_dependence = CO_curves[iteration]
        env.time_input_dependence = lambda act, t: np.array([act[0], CO_curves[iteration](t)])
        return True

    # fourier_dependence, fourier_callback = get_fourier_time_dependence(5, episode_time)
    rtp_dependence, rtp_callback = get_random_turns_time_dependence(10., (0., 1.), 1.)
    ...
    run_jobs_list(**get_for_RL_iterations(),
                  **jobs_list_from_grid(
                      ...
                      [(0.14895, 0.06594 * 2, 0.1), (0.14895, 0.06594 * 4, 0.1),
                       (1.4895, 0.6594 * 2, 1.), (1.4895, 0.6594 * 4, 1.),
                       ],
                  ...
                  python_interpreter='../RL_10_21/venv/bin/python',
                  out_fold_path='./run_RL_out/LibudaG/230713_CO(t)_RTP_vpg_optimization',
                  at_same_time=110,
                  )
