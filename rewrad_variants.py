...
def get_reward_func(params: dict):
    if params['name'] == 'each_step_normalize':
    ...
    elif params['name'] == 'each_step_1':

        def each_step_1(env_obj) -> float:
            raise NotImplementedError

        out_func = each_step_1
    ...
    elif params['name'] == 'full_ep_1':

        def full_ep_1(env_obj) -> float:
            raise NotImplementedError
        ...
        def full_ep_2(env_obj) -> float:
            ...
            #     int_arr = env_obj.integral_plot['integral'][:env_obj.count_episodes]
            #     std_lasts = np.std(int_arr[-depth:])
            #     if std_lasts < 5.e-2 * env_obj.normalize_koef:
            #         std_lasts = 5.e-2 * env_obj.normalize_koef
        ...
        def hybrid_reward(env_obj) -> float:
            ...
            Reward combines full_ep_[mode] and each_step_1 rewards
