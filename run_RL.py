...
def debug_run(environment: RL2207_Environment, agent, out_folder=None, n_episodes=10):
    ...
    for i in range(n_episodes):
        ...
        if i == n_episodes - 2:
            ...
            environment.controller.plot(f'{subdir_path}/{environment.integral:.2f}conversion{i}.png', additional_plot='D', plot_mode='separately',
                                        out_name='target')
    agent = Agent.load(directory=f'{subdir_path}/agent', format='numpy', environment=environment)
    run_episode(environment, agent)
    environment.controller.plot(f'{subdir_path}/{environment.integral:.2f}conversion_test.png', additional_plot='D', plot_mode='separately',
                                out_name='target')
    ...
    for i in range(n_episodes):
        ...
        if not (i % freq) or (i > n_episodes - 5):
            ...
            environment.controller.plot(f'{dir_path}/{environment.integral:.2f}conversion{i}.png',
                                additional_plot='D', plot_mode='separately', out_name='target')
